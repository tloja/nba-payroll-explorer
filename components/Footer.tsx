...
export function Footer() {
  return (
    <footer className="mx-auto mt-12 max-w-5xl px-4 py-6">
      <nav aria-label="Site" className="mb-3 flex flex-wrap gap-x-4 gap-y-1 text-xs">
        {LINKS.map((link) => (
          <Link key={link.href} href={link.href} className="text-[#52514e] underline">
            {link.label}
          </Link>
        ))}
      </nav>
      <p className="border-t border-[#d8d6cf] pt-3 text-xs text-[#52514e]">{TRADEMARK_DISCLAIMER}</p>
    </footer>
  );
}
